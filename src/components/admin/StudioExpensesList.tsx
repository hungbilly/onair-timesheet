...
interface StudioExpensesListProps {
  refreshTrigger: number;
  selectedMonth: string;
}

const StudioExpensesList = ({ refreshTrigger, selectedMonth }: StudioExpensesListProps) => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        ...
        toast({
          title: "Error",
          description: "Failed to load studio expenses",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        title: "Success",
        description: "Studio expense deleted successfully",
      });
    } catch (error) {
  ...
  const handleExpenseUpdated = (updatedExpense: StudioExpense) => {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="py-8 text-center">No studio expenses found for this month</div>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
...
export default StudioExpensesList;
